        program calc_indices

!	ATENCAO!!!... Este programa deve ser compilado na io para aceitar
! 		      a dimensao  MATR(nx,ny).       

! 	Este programa calcula a CAPE e a CINE prevista pelo modelo RAMS.

 !	Demerval  e Bruno utilizando a subrotina do Marcos - 11/02/2003

      character nomeCTL*100,nomeOUT*100,nomeOUT2*100,nomeCTL_orig*100
      character var*100,nomeIN*100
      real indef

      Print*,"ENTRE COM O VALOR INDEFINIDO:"
      read(*,*) indef
      Print*,"ENTRE COM O NUMERO DE PONTOS: NX, NY, NZ, NT:"
      read (*,*) nx,ny,nz,nt
      print*,"verifica se o arquivo grade_cape.h estah com os pontos:",nx,ny
      Print*,"ENTRE COM nSz nSzp PARA PRESS:"
      read (*,*) nSzP,nSzpP
      Print*,"ENTRE COM nSz nSzp PARA TEMPK:"
      read (*,*) nSzT,nSzpT
      Print*,"ENTRE COM nSz nSzp PARA RH:"
      read (*,*) nSzUR,nSzpUR
      Print*,"ENTRE COM nomeIN (.gra):"
      read (*,'(a100)') nomeIN
      Print*,"O arquivo gra eh little_endian (1 p/ sim):"
      read (*,*) lit
      lit=0

      do i=1,100
        if (nomeIN(i:i).ne.' ') icont=i
      enddo
      nomeOUT='/TEMPO2/www/rams/capecineb_'//nomeIN(icont-14:icont)
      nomeOUT2='capecineb_'//nomeIN(icont-14:icont)

      call exec(nz,nt,nomeIN,nomeOUT,&
               nSzP,nSzpP,nSzT,nSzpT,nSzUR,nSzpUR,lit,indef)

!**************************** CONSTROI CTL ****************************
!      nomeCTL_orig=nomeIN(1:icont-3)//'ctl'
!      open (10,file=nomeCTL_orig,status='old')
!      nomeCTL='/TEMPO2/www/rams/capecine_'//nomeIN(icont-14:icont-3)//'ctl'
!      open (11,file=nomeCTL)
! 100  read(10,'(a)') var
!        if (var(1:4).eq.'dset') write(11,'(a6,a)') 'dset ^',nomeOUT2
!        if (var(1:5).eq.'optio') write(11,'(a)') 'options little_endian'
!        if (var(1:5).eq.'undef'.or.var(1:5).eq.'title'
!     &     .or.var(1:4).eq.'xdef'.or.
!     &     var(1:4).eq.'ydef'.or.var(1:4).eq.'tdef') write(11,'(a)') var
!        if (var(1:4).eq.'zdef') write(11,'(a)') 'zdef   1 levels   500.' 
!      if (var(1:4).ne.'vars') goto 100
!      write(11,'(a)') 'vars    2'
!      write(11,'(a)') 'cape    0 99  -   Cape'
!      write(11,'(a)') 'cine    0 99  -  cine'
!      write(11,'(a)') 'endvars'
